...
Namespace SoundcloudPlayer
    Friend MustInherit Class ThemeControl154
        ...
        Protected Function Center(ByVal child As Rectangle) As Point
            Return Me.Center(Me.Width, Me.Height, child.Width, child.Height)
        End Function
        ...
        Protected Function Center(ByVal childWidth As Integer, ByVal childHeight As Integer) As Point
            Return Me.Center(Me.Width, Me.Height, childWidth, childHeight)
        End Function
        ...
        Protected Sub DrawBorders(ByVal p1 As Pen)
            Me.DrawBorders(p1, 0, 0, Me.Width, Me.Height)
        End Sub
        ...
        Protected Sub DrawBorders(ByVal p1 As Pen, ByVal offset As Integer)
            Me.DrawBorders(p1, 0, 0, Me.Width, Me.Height, offset)
        End Sub
        ...
        Protected Sub DrawCorners(ByVal c1 As Color)
            Me.DrawCorners(c1, 0, 0, Me.Width, Me.Height)
        End Sub
        ...
        Protected Sub DrawCorners(ByVal c1 As Color, ByVal offset As Integer)
            Me.DrawCorners(c1, 0, 0, Me.Width, Me.Height, offset)
        End Sub
        ...
        Protected Sub DrawGradient(ByVal blend As ColorBlend, ByVal r As Rectangle)
            Me.DrawGradientBrush = New LinearGradientBrush(r, Color.Empty, Color.Empty, 90.0!)
        ...
        Protected Sub DrawGradient(ByVal c1 As Color, ByVal c2 As Color, ByVal r As Rectangle)
            Me.DrawGradientBrush = New LinearGradientBrush(r, c1, c2, 90.0!)
            Me.G.FillRectangle(Me.DrawGradientBrush, r)
        End Sub
            ...
            If (Not image Is Nothing) Then
                ...
                Select Case CInt(a)
                    Case 0
                        ...
                        Exit Select
                    Case 1
                        Me.G.DrawImage(image, ((Me.Width - image.Width) - x), (Me.DrawImagePoint.Y + y), image.Width, image.Height)
                        Exit Select
                    Case 2
        ...
        Protected Sub DrawRadial(ByVal c1 As Color, ByVal c2 As Color, ByVal r As Rectangle)
            Me.DrawRadialBrush2 = New LinearGradientBrush(r, c1, c2, 90.0!)
        ...
        Public Sub DrawRadial(ByVal blend As ColorBlend, ByVal r As Rectangle, ByVal cx As Integer, ByVal cy As Integer)
            ...
            Dim point As New Point((r.X + cx), (r.Y + cy))
            ' Me.DrawRadialBrush1.CenterPoint = DirectCast(point, PointF)
            DrawRadialBrush1.CenterPoint = New Point(r.X + cx, r.Y + cy)
            Me.DrawRadialBrush1.InterpolationColors = blend
            If (Me.G.SmoothingMode = SmoothingMode.AntiAlias) Then
                Me.G.FillEllipse(Me.DrawRadialBrush1, CInt((r.X + 1)), CInt((r.Y + 1)), CInt((r.Width - 3)), CInt((r.Height - 3)))
            Else
            ...
            If ([text].Length <> 0) Then
                ...
                Select Case CInt(a)
                    Case 0
                        ...
                        Exit Select
                    Case 1
                        Me.G.DrawString([text], Me.Font, b1, CSng(((Me.Width - Me.DrawTextSize.Width) - x)), CSng((Me.DrawTextPoint.Y + y)))
                        Exit Select
                    Case 2
        ...
        Protected Function GetBrush(ByVal name As String) As SolidBrush
            Return New SolidBrush(Me.Items.Item(name))
        End Function

        Protected Function GetColor(ByVal name As String) As Color
            Return Me.Items.Item(name)
        End Function

        Protected Function GetPen(ByVal name As String) As Pen
            Return New Pen(Me.Items.Item(name))
        End Function

        Protected Function GetPen(ByVal name As String, ByVal width As Single) As Pen
            Return New Pen(Me.Items.Item(name), width)
        End Function

        Private Sub InvalidateBitmap()
            If ((Me.Width <> 0) AndAlso (Me.Height <> 0)) Then
                Me.B = New Bitmap(Me.Width, Me.Height, PixelFormat.Format32bppPArgb)
                Me.G = Graphics.FromImage(Me.B)
            End If
        End Sub

        Private Sub InvalidateCustimization()
            Dim stream As New MemoryStream((Me.Items.Count * 4))
            ' Dim bloom As Bloom
            'For Each bloom In Me.Colors
            'stream.Write(BitConverter.GetBytes(bloom.Value.ToArgb), 0, 4)
            'Next
            stream.Close()
            Me._Customization = Convert.ToBase64String(stream.ToArray)
        End Sub

        Private Sub InvalidateTimer()
            If (Not Me.DesignMode AndAlso Me.DoneCreation) Then
                If Me._IsAnimated Then
                    ThemeShare.AddAnimationCallback(New ThemeShare.AnimationDelegate(AddressOf Me.DoAnimation))
                Else
                    ThemeShare.RemoveAnimationCallback(New ThemeShare.AnimationDelegate(AddressOf Me.DoAnimation))
                End If
            End If
        End Sub

        Protected Function Measure() As Size
            Return Me.MeasureGraphics.MeasureString(Me.Text, Me.Font, Me.Width).ToSize
        End Function

        Protected Function Measure(ByVal [text] As String) As Size
            Return Me.MeasureGraphics.MeasureString([text], Me.Font, Me.Width).ToSize
        End Function
        ...
        Protected Overrides Sub OnEnabledChanged(ByVal e As EventArgs)
            If Me.Enabled Then
                Me.SetState(MouseState.None)
            Else
                Me.SetState(MouseState.Block)
            End If
        ...
        Protected NotOverridable Overrides Sub OnHandleCreated(ByVal e As EventArgs)
            Me.InvalidateCustimization()
            Me.ColorHook()
            If (Me._LockWidth <> 0) Then
                Me.Width = Me._LockWidth
            End If
            If (Me._LockHeight <> 0) Then
                Me.Height = Me._LockHeight
            End If
            Me.Transparent = Me._Transparent
            If (Me._Transparent AndAlso Me._BackColor) Then
        ...
        Public Overrides Property BackColor As Color
            ...
            Set(ByVal value As Color)
                If (Not Me.IsHandleCreated AndAlso (value = Color.Transparent)) Then
                    Me._BackColor = True
                Else
        ...
        Public Property Colors As Bloom()
            'Get
            '    Dim list As New List(Of Bloom)
            '    Dim Enumerator As Enumerator(Of String, Color) = Me.Items.GetEnumerator
            '    Do While enumerator.MoveNext
            '        Dim current As KeyValuePair(Of String, Color) = enumerator.Current
            '        Dim pair2 As KeyValuePair(Of String, Color) = enumerator.Current
            '        Dim item As New Bloom(current.Key, pair2.Value)
            '        list.Add(item)
            '    Loop
            '    Return list.ToArray
            'End Get
            ...
            '    Dim bloom As Bloom
            '    For Each bloom In value
            '        If Me.Items.ContainsKey(bloom.Name) Then
            '            Me.Items.Item(bloom.Name) = bloom.Value
            '        End If
            '    Next
            '    Me.InvalidateCustimization()
            '    Me.ColorHook()
        ...
        Public Property Customization As String
            ...
            Set(ByVal value As String)
                If (value <> Me._Customization) Then
                    Dim colors As Bloom() = Me.Colors
                    Try
                        Dim buffer As Byte() = Convert.FromBase64String(value)
                        Dim num2 As Integer = (colors.Length - 1)
                        Dim i As Integer = 0
                        Do While (i <= num2)
        ...
        Protected Property LockHeight As Integer
            ...
            Set(ByVal value As Integer)
                ...
                If ((Me.LockHeight <> 0) AndAlso Me.IsHandleCreated) Then
                    Me.Height = Me.LockHeight
                End If
            End Set
        End Property
        ...
        Protected Property LockWidth As Integer
            ...
            Set(ByVal value As Integer)
                Me._LockWidth = value
                If ((Me.LockWidth <> 0) AndAlso Me.IsHandleCreated) Then
                    Me.Width = Me.LockWidth
                End If
            End Set
        End Property
        ...
        Public Property Transparent As Boolean
            ...
            Set(ByVal value As Boolean)
                ...
                If Me.IsHandleCreated Then
                    If (Not value AndAlso (Me.BackColor.A <> &HFF)) Then
                        Throw New Exception("Unable to change value to false while a transparent BackColor is in use.")
                    End If
                    Me.SetStyle(ControlStyles.Opaque, Not value)
                    Me.SetStyle(ControlStyles.SupportsTransparentBackColor, value)
                    If value Then
                        Me.InvalidateBitmap()
                    Else
                        Me.B = Nothing
        ...
        Private Items As Dictionary(Of String, Color) = New Dictionary(Of String, Color)
